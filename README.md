...
1) В папке Shoots_normalization находится все необходимое для нормализации изображений (и сами результаты нормализации):
    ...
    - В папке result находятся уже нормализованные изображения (но старые варианты, перед запуском алгоритмов стоит заново нормализовать исходники).
...
Для запуска проекта необходимо установить opencv, glm и компилятор c++. Подключить opencv и glm к проекту и скомпилировать файл Uncontrolled_mosaic.cpp вместе со всеми дополнительными библиотеками .h, которые были рассмотрены выше и содержат необходимые алгоритмы, нормализации и т.д.
